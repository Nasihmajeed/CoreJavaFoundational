...
public void editList(Contact contac,String[] temp){
	...
	System.out.println("######"+contac.getName());
	try{
		String a=temp[1];
		String b=temp[2];
		String c=temp[0];
...
/*		contac.setId(Integer.parseInt(c));
		contac.setName(a);
		contac.setName(b);
		colist.set(i,contac);*/
	    }
...
public void deleteList(String cn){
	try{
		...
		con=DriverManager.getConnection("jdbc:mysql://localhost:3306/contacts","root","root");
		Statement s=con.createStatement();
		ResultSet rs=s.executeQuery("delete * from contactlist where(NAME ='"+cn+"')");	
       }
    catch(Exception ee){
		System.out.println(ee);}
